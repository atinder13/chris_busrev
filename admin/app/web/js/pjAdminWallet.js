	...
	$(function () {
			...
			$dialogSelect.dialog({
				...
				open: function (){
					if($('#reload_map').val() == '1' )
					{
						...
						$.post("index.php?controller=pjAdminWallet&action=pjActionGetSeats", $frm.serialize()).done(function (data) {
			...
			$dialogReturnSelect.dialog({
				...
				open: function (){
					if($('#return_reload_map').val() == '1' )
					{
						...
						$.post("index.php?controller=pjAdminWallet&action=pjActionGetReturnSeats", $frm.serialize()).done(function (data) {
		...
		if ($("#grid").length > 0 && datagrid) {
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "#"},
				         ],
				columns: [
				          {text: myLabel.operator, type: "text", sortable: false, width:140},
				          {text: myLabel.bookingpay, type: "text", sortable: false, editable: false, width:130},
				          {text: myLabel.givepay, type: "text", sortable: false, editable: false},
				          {text: myLabel.amount, type: "text", sortable: false, editable: false},
				         ],
				dataUrl: "index.php?controller=pjAdminWallet&action=pjActionGetBooking" + pjGrid.queryString,
				dataType: "json",
				fields: ['operator', 'bookingpay', 'givepay', 'amount'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "index.php?controller=pjAdminWallet&action=pjActionDeleteBookingBulk", render: true, confirmation: myLabel.delete_confirmation},
					   {text: myLabel.exported, url: "index.php?controller=pjAdminWallet&action=pjActionExportBooking", ajax: false}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "index.php?controller=pjAdminWallet&action=pjActionSaveBooking&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("focusin", ".datepick", function (e) {
				...
				o = {
					...
					onSelect: function (dateText, inst) {
						if($this.attr("id") == 'booking_date'){
							...
							$.post("index.php?controller=pjAdminWallet&action=pjActionChangeDate", frm.serialize()).done(function (data) {
			...
			$grid.datagrid("load", "index.php?controller=pjAdminWallet&action=pjActionGetBooking", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$('.bs-loader').css('display', 'block');
			$.get("index.php?controller=pjAdminWallet&action=pjActionGetLocations&pickup_id=" + $(this).val()).done(function (data) {
			...
			{
				$('.bs-loader').css('display', 'block');
				$.get("index.php?controller=pjAdminWallet&action=pjActionGetLocations&return_id=" + $(this).val()).done(function (data) {
			...
			if($('#bus_id'). val() != '')
			{
				...
				$('.bs-loader').css('display', 'block');
				$.post("index.php?controller=pjAdminWallet&action=pjActionGetTickets", frm.serialize()).done(function (data) {
			...
			if($(this). val() != '')
			{
				...
				$('.bs-loader').css('display', 'block');
				$.post("index.php?controller=pjAdminWallet&action=pjActionGetReturnTickets", frm.serialize()).done(function (data) {
		...
		function getBuses()
		{
			if($('#pickup_id'). val() != '' && $('#return_id'). val() != '')
			{
				...
				$('.bs-loader').css('display', 'block');
				$.post("index.php?controller=pjAdminWallet&action=pjActionGetBuses", frm.serialize()).done(function (data) {
					$('#busBox').html(data);
					$('.bs-loader').css('display', 'none');
		...
		function getReturnBuses()
		{
			if($('#pickup_id'). val() != '' && $('#return_id'). val() != '')
			{
				...
				$.post("index.php?controller=pjAdminWallet&action=pjActionGetReturnBuses", frm.serialize()).done(function (data) {
					$('#returnBox').html(data);
