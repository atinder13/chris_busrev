...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBuses&amp;action=pjActionTime" method="post" id="frmUpdateTime" class="pj-form form">
		...
		<p>
				<label class="title">Bus Number:</label>
				<span class="inline_block" id="boxBusOperator">
					
				<input type="text" value="<?php echo $tpl['arr']['bus_no'] ?>" class="pj-form-field w200 required" name="bus_no" id="bus_no" />
				</span>
			</p>
			...
			<p>
				...
				<input type="text" value="<?php echo $tpl['arr']['color'] ?>" class="pj-form-field w200 required" name="color" id="color" />
				</span>
			</p>
		...
		<p>
				<label class="title">Amenities:</label>
				<span class="inline_block">
					<?php
					foreach ($GLOBALS['DBAmenities'] as $Key => $Amenitie) {
						if(in_array($Key, $tpl['AMData'])){
						$chk="checked";
						}
						else{
							$chk="";
					...
					?>
				<span class="block b5"><input type="checkbox" id="amenitie" name="amenitie[]" value="<?php echo $Key ?>"  class="r5" <?php echo $chk ?> /><label for="bs_weekday_monday"><?php echo $Amenitie ;?></label></span>
				<?php } ?>
				</span>
			</p>
		<p>
