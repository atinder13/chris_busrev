...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBusTypes&amp;action=pjActionUpdate" method="post" id="frmUpdateBusType" class="pj-form form" enctype="multipart/form-data">
		...
		<div class="clear_both">
			...
			{
				$OldNames=explode("_",$tpl['arr']['i18n'][$v['id']]['name']);
			$OldName=(count($OldNames)>1) ? end($OldNames) : $OldNames[0];
			?>
				<p class="pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
					<label class="title"><?php __('lblName'); ?>:</label>
					<span class="inline_block">
						<input type="text" name="i18n[<?php echo $v['id']; ?>][name]" class="pj-form-field w300<?php echo (int) $v['is_default'] === 0 ? NULL : ' required'; ?>" value="<?php echo htmlspecialchars(stripslashes(@$OldName)); ?>"/>
						<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
				...
				<span class="inline_block" id="boxBusOperator">
					<select name="operator_id" id="operator_id" class="pj-form-field w250 required">
						...
						foreach ($tpl['Users'] as $v)
						{
							...
							?><option value="<?php echo $v['id']; ?>" <?php echo $s ?>><?php echo stripslashes($v['name']); ?></option><?php
						}
						?>
					</select>
