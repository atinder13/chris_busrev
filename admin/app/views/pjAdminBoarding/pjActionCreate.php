...
// pjUtil::printNotice(__('infoAddRouteTitle', true, false), __('infoAddRouteDesc', true, false));
	...
	?>
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminBoarding&amp;action=pjActionCreate" method="post" id="frmCreateRoute" class="pj-form form">
		<input type="hidden" name="boarding_create" value="1" />
				...
				<div id="bs_location_list" class="bs-location-list">
					...
					if(!isset($_GET['id']))
					{
						...
						<div class="bs-location-row" data-index="<?php echo $index;?>">
							<p>
								<label class="title bs-title-<?php echo $index;?>">Boarding/Drop-off 1:</label>
							
								<span class="inline_block">
						<input type="text" name="boarding[]" class="pj-form-field w300<?php echo (int) $v['is_default'] === 0 ? NULL : ' required'; ?>" lang="<?php echo $v['id']; ?>" />
						<?php if ((int) $tpl['option_arr']['o_multi_lang'] === 1 && count($tpl['lp_arr']) > 1) : ?>
