...
<table class="pj-table" cellspacing="0" cellpadding="0" style="width: 100%;">
	<thead>
		<tr>
			...
			<th>
				<div class="pj-table-sort-label">Operator</div>
				<div class="pj-table-sort">
					<a href="#" class="pj-table-sort-up<?php echo $column == 'opname' && $direction == 'ASC' ? ' pj-table-sort-up-active' : null;?>" data-column="opname"></a>
					<a href="#" class="pj-table-sort-down<?php echo $column == 'opname' && $direction == 'DESC' ? ' pj-table-sort-down-active' : null;?>" data-column="opname"></a>
				</div>
			</th>
			<th style="width: 80px;">
	...
	<tbody>
		...
		if(count($tpl['bus_arr']) > 0)
		{
			foreach($tpl['bus_arr'] as $v)
			{
				...
				list($departure_time, $departure_id) = (isset($v['departure'])) ? explode("~:~", $v['departure']) : '';
				list($arrival_time, $arrival_id) = (isset($v['departure'])) ? explode("~:~", $v['arrive']) : '';
				?>
				<tr>
					...
					<td class="<?php echo $total_tickets > 0 ? 'pj-table-row-clickable' : null;?>" <?php echo $total_tickets > 0 ? 'data-href="' .$_SERVER['PHP_SELF'] . '?controller=pjAdminSchedule&amp;action=pjActionBookings&amp;bus_id=' . $v['id'] . '&amp;date='.$date.'"' : null;?>><?php echo pjSanitize::clean($v['opname']);?></td>
