...
class pjAdminWallet extends pjAdmin
{
	...
	public function pjActionIndex()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			$this->appendJs('pjAdminWallet.js');
	...
	public function pjActionGetBookingOld()
	{
		$this->setAjax(true);
	
		if ($this->isXHR())
		{	
			$EditorId=$_SESSION['admin_user']['id'];
			$pjBookingModel = pjBookingModel::factory()
				...
				->join('pjUser', "t6.id=t2.operator_id", 'left outer')
				->join('pjWallet', "t7.uuid=t1.uuid", 'left outer')
				;
			...
			$pjBookingModel->where("t2.operator_id != '' ");
			$column = 'created';
			$direction = 'DESC';
			...
			{
				$data = $pjBookingModel
				->select('t1.*,t2.operator_id, t2.departure_time, t2.arrival_time, t3.content as route_title, t4.content as from_location, t5.content as to_location,t6.name,t7.status as payStatus,SUM(t1.total) as totalAmt')
				->orderBy("$column $direction")
				->groupBy('t2.operator_id')
			...
			foreach($data as $k => $v)
			{
				...
				$v['date_time'] = pjUtil::formatDate($v['booking_date'], 'Y-m-d', $this->option_arr['o_date_format']) . '<br/>' . $v['booking_time'];
				...
				$v['amount'] = $v['totalAmt'];
				$v['Paystatus'] = $Paystatus;
				$v['email'] = $_SESSION['admin_user']['email'];
				$v['adminname'] = $_SESSION['admin_user']['name'];
				$v['opdes']= $v['name'] .' Payment';
				$data[$k] = $v;
			}
	...
	public function pjActionGetBooking()
	{
		...
		if ($this->isXHR())
		{
			...
			$DPay = pjBookingModel::factory()
				->join('pjBus', "t2.id=t1.bus_id", 'left outer')
				->select('SUM(t1.total) as dpay')
				->where('t2.operator_id',$v['id'])
				->groupBy('t2.operator_id')
				->findAll()
				->getData();
			$LPay = pjWalletModel::factory()
				 ->select('SUM(t1.payment) as lpay')
				->where('t1.operator_id',$v['id'])
				->groupBy('t1.operator_id')
				->findAll()
				->getData();
				
			
			if(isset($DPay[0]['dpay']) && $DPay[0]['dpay'] > 0){
				$Debit=$DPay[0]['dpay'];
			}
			else{
				$Debit=0;
			...
			if(isset($LPay[0]['lpay']) && $LPay[0]['lpay'] > 0){
				$Credit=$LPay[0]['lpay'];
			}
			else{
				$Credit=0;
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (isset($_POST['booking_update']))
			{
				...
				$data['booking_datetime'] = $booking_date;
				if(isset($_SESSION[$this->defaultPeriod]))
				{
					$data['booking_datetime'] = $_SESSION[$this->defaultPeriod]['departure_time'];
					$data['stop_datetime'] = $_SESSION[$this->defaultPeriod]['arrival_time'];
