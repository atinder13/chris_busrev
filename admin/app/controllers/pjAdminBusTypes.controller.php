...
class pjAdminBusTypes extends pjAdmin
{
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			$pjUserModel->where('t1.id', $_POST['operator_id']);
			$Users = array();
			$Users = $pjUserModel->select('t1.id, t1.name')->findAll()->getData();
			$Uname=str_replace(' ', '', $Users[0]['name']);
			$_POST['i18n'][1]['name']=$Uname.'_'.$_POST['i18n'][1]['name'];
					...
					if(empty($_FILES['image']['tmp_name']) && isset($_POST['defaultmapimg'])) {
							$data = array();
							$data['seats_map'] = 'app/web/upload/bus_types/'.$_POST['defaultmapimg'];
																									
						$pjBusTypeModel->reset()->where('id', $id)->limit(1)->modifyAll($data);
					}

					if (empty($_FILES['image']['tmp_name']) && empty($_POST['defaultmapimg']))
					{
	...
	public function pjActionGetBusType()
	{
		...
		if ($this->isXHR())
		{
			...
			$pjBusTypeModel = pjBusTypeModel::factory()
							->join('pjMultiLang', "t2.model='pjBusType' AND t2.foreign_id=t1.id AND t2.field='name' AND t2.locale='".$this->getLocaleId()."'", 'left outer')
							->join('pjUser', "t3.id=t1.operator_id AND t3.role_id='2'", 'left outer');
			...
			if ($this->isEditor()){
			$pjBusTypeModel->where("t1.operator_id",$EditorId);
			}
			...
			if ($this->isAdmin())
			{
				...
				$data = $pjBusTypeModel->select("t1.id, t1.seats_map, t1.seats_count, t1.status, t2.content as name,t3.name as opname")->orderBy("$column $direction")->limit($rowCount, $offset)->findAll()->getData();
			}
			else{
				
			$data = $pjBusTypeModel->select("t1.id, t1.seats_map, t1.seats_count, t1.status, t2.content as name,t3.name as opname")->orderBy("$column $direction")->where("t1.operator_id",$EditorId)->limit($rowCount, $offset)->findAll()->debug(true)->getData();
			}
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			$OldNames=explode("_",$_POST['i18n'][1]['name']);
			$OldName=(count($OldNames)>1) ? end($OldNames) : $OldNames[0];
			$_POST['i18n'][1]['name']=$Uname.'_'.$OldName;
	...
	public function pjActionDeleteMap()
	{
		...
		if ($this->isXHR())
		{
			...
			$imgName= basename($arr['seats_map']);
		
			if(!empty($arr))
			{
				$map_path = $arr['seats_map'];
				if (file_exists(PJ_INSTALL_PATH . $map_path) && !in_array($imgName, $DefaultImg)) {
