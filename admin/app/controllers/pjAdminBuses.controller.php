	...
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (isset($_POST['bus_create']))
			{
				...
				{
					if(count($_POST['amenitie']) > 0){
					$pjAmenitie= pjAmenitieModel::factory();
					foreach ($_POST['amenitie'] as  $amenitie) {

					$pjAmenitie->reset()->setAttributes(array('amId'=>$amenitie,'bus_id'=>$id))->insert();
					}
				}
	...
	public function pjActionGetBus()
	{
		...
		if ($this->isXHR())
		{
			...
			if (isset($_GET['q']) && !empty($_GET['q']))
			{
				...
				$pjBusModel->where("(t1.content LIKE '%$q%' OR t3.content LIKE '%$q%')");
			}
			...
			if ($this->isEditor()){
			$pjBusModel->where("t1.operator_id",$EditorId);
			}
			...
			$data = $pjBusModel->select(" t1.*, t2.content AS route,t3.name as opname")->orderBy("$column $direction")->limit($rowCount, $offset)->findAll()->getData();
			}
			else{
				$data = $pjBusModel->select(" t1.*, t2.content AS route,t3.name as opname")->where("t1.operator_id",$EditorId)->orderBy("$column $direction")->limit($rowCount, $offset)->findAll()->getData();
			}
	...
	public function pjActionTime()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			if (isset($_POST['bus_update']))
			{
				...
				$pjAmenitie->reset()->where('bus_id', $_POST['id'])->eraseAll();
				
				if(count($_POST['amenitie']) > 0){
					
					foreach ($_POST['amenitie'] as  $amenitie) {

					$pjAmenitie->reset()->setAttributes(array('amId'=>$amenitie,'bus_id'=>$_POST['id']))->insert();
					}
				}
				...
				$location_arr = pjRouteCityModel::factory()->select('t1.*, t2.content as name')
								...
								->where('route_id', $arr['route_id'])->orderBy("t1.order ASC")->debug(true)->findAll()->getData();
			...
			$pjAmenitiesArr= pjAmenitieModel::factory()
			->where('bus_id', $_GET['id'])
					->findAll()
					->getData();
			$AMData=array();
			if(count($pjAmenitiesArr)>0){			
			foreach ($pjAmenitiesArr as $AM) {
					$AMData[]=$AM['amId'];
			}
