	...
	public function pjActionGetRoute()
	{
		...
		if ($this->isXHR())
		{
			...
			$pjRouteModel = pjRouteModel::factory()
							...
							->join('pjUser', "t5.id=t1.operator_id AND t5.role_id='2'", 'left outer');;
			...
			if ($this->isEditor()){
			$pjRouteModel->where("t1.operator_id",$EditorId);
			}
			...
			{
				$data = $pjRouteModel
				->select(" t1.id, t1.status, t2.content as title, t3.content as `from`, t4.content as `to`,t5.name as opname")
