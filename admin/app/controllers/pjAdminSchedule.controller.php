		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			if($route_id != null)
			{
				$booking_arr = pjBookingModel::factory()
					->select("t1.bus_id, t1.booking_date, (SELECT SUM(TBT.qty) 
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if(isset($_GET['bus_id']) && isset($_GET['date']))
			{
				...
				$ticket_arr = pjBookingTicketModel::factory()
					...
					->where("t1.booking_id IN (SELECT TB.id FROM `".pjBookingModel::factory()->getTable()."` AS TB WHERE TB.status='confirmed' AND TB.bus_id = ".$bus_id." AND TB.booking_date='$date')")
	...
	public function pjActionGetSchedule()
	{
		...
		if ($this->isXHR())
		{
			...
			$pjBusModel = pjBusModel::factory()
					...
					->join('pjUser', "t6.id=t1.operator_id AND t6.role_id='2'", 'left outer');			
			...
			$bus_arr = $pjBusModel->select(" t1.*, t5.content AS route,,t6.name as opname,											  														  
	...
	public function pjActionGetBookings()
	{
		...
		if ($this->isXHR())
		{
			...
			$ticket_arr = pjBookingTicketModel::factory()
				->select("t1.ticket_id, t2.content as title, SUM(qty) AS total_tickets")
				->join('pjMultiLang', "t2.model='pjTicket' AND t2.foreign_id=t1.ticket_id AND t2.field='title' AND t2.locale='".$this->getLocaleId()."'", 'left outer')
				->where("t1.booking_id IN (SELECT TB.id FROM `".pjBookingModel::factory()->getTable()."` AS TB WHERE TB.bus_id = ".$bus_id." AND TB.booking_date='$date'$and_where)")
				->groupBy("t1.ticket_id")
				->findAll()->getData();
	...
	public function pjActionGetTimetable()
	{
		...
		if ($this->isXHR())
		{
			...
			$pjBusModel = pjBusModel::factory()
					->join('pjMultiLang', "t2.model='pjRoute' AND t2.foreign_id=t1.route_id AND t2.field='title' AND t2.locale='".$this->getLocaleId()."'", 'left outer')
					->join('pjBusType', "t3.id=t1.bus_type_id", 'left outer')
					->join('pjUser', "t4.id=t1.operator_id AND t4.role_id='2'", 'left outer');
			...
			$column = 'departure_time';
			$direction = 'ASC';
			$bus_arr = $pjBusModel->select(" t1.*, t2.content AS route,t4.name as opname")
				->orderBy("$column $direction")
				->findAll()
				->getData();
						...
						if(pjUtil::checkDateFormat($date))
						{
							...
							$ticket_arr = pjBookingTicketModel::factory()
								...
								->where("t1.booking_id IN (SELECT TB.id 
														   ...
														   WHERE TB.status='confirmed' AND TB.bus_id = ".$bus_id." AND TB.booking_date='$date'$and_where)")
								->groupBy("t1.ticket_id")
								->findAll()->getData();
