...
class pjAdminBusOperator extends pjAdmin
{
	public function pjActionCreate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			...
			{
				pjUtil::redirect(PJ_INSTALL_URL . "index.php?controller=pjAdminBusOperator&action=pjActionIndex&err=ABT05");
			}
			if (isset($_POST['bus_type_create']))
			{
				$pjBusTypeModel = pjBusOperatorModel::factory();
				...
				if ($id !== false && (int) $id > 0)
				{
					...
					if (isset($_FILES['seats_map']))
					{
						if($_FILES['seats_map']['error'] == 0)
						{
							if (is_writable('app/web/upload/bus_types'))
							{
								...
								pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminBusOperator&action=pjActionUpdate&id=$id&err=ABT11");
							}
						}else if($_FILES['seats_map']['error'] != 4){
							pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminBusOperator&action=pjActionUpdate&id=$id&err=ABT09");
						}
					}
					...
					pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminBusOperator&action=pjActionUpdate&id=$id&err=$err");
				} else {
					$err = 'ABT04';
					pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjAdminBusOperator&action=pjActionIndex&err=$err");
				}
	...
	public function pjActionGetBusType()
	{
		...
		if ($this->isXHR())
		{
			...
			if (isset($_GET['q']) && !empty($_GET['q']))
			{
				...
				$pjBusTypeModel->where('t1.operator LIKE', "%$q%");
			}
